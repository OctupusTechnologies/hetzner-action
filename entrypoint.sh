...
if [ -f "cloud-config.yaml" ]; then
  ...
  output=$(hcloud server create --image 134385409 --name $SERVER_NAME --type $SEVER_TYPE --firewall $HCLOUD_FIREWALL --datacenter $HETZNER_DATACENTER --ssh-key $HCLOUD_SSH_KEY --user-data-from-file cloud-config.yaml --without-ipv6  )
else
  echo "Create server without cloud-config"
  output=$(hcloud server create --image 134385409 --name $SERVER_NAME --type $SEVER_TYPE --firewall $HCLOUD_FIREWALL --datacenter $HETZNER_DATACENTER --ssh-key $HCLOUD_SSH_KEY --without-ipv6 )
fi
echo $output
